...
export function buildCodegenPrompt(ir: IR): string {
  ...
  return [
    ...
    '',
    'TypeScript OOM Guard (MANDATORY for src/server.ts)',
    '1) Add this helper after importing zodToJsonSchema:',
    ...
    '   }',
    '2) For every tool definition, use: inputSchema: toInputSchema(mySchema)',
    '3) NEVER use: zodToJsonSchema(mySchema) as Tool["inputSchema"]',
    '4) Ensure generated src/server.ts contains this helper and uses it for every tool inputSchema.',
    '',
